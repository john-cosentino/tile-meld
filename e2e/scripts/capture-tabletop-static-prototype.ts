// Static tabletop concept prototype -- screenshot capture + route-load
// smoke check (docs/meld-masters-tabletop-static-prototype-summary.md).
// Unlike every capture-phase-N-review.ts / capture-tabletop-fidelity.ts
// script, this one needs NO room/login setup at all -- the route is fully
// static mock data, reachable directly. Doubles as this checkpoint's
// required "Chromium route-load check" and "Mobile Chrome route-load
// check": each context asserts the masthead/board/rack/action-bar are
// actually visible before any screenshot is taken.
//
// Usage: pnpm exec tsx e2e/scripts/capture-tabletop-static-prototype.ts [outDir]
...
async function assertRouteLoaded(page: Page, label: string): Promise<void> {
  ...
  await page.locator(".concept-masthead").waitFor({ state: "visible", timeout: 15000 });
...
async function run(): Promise<void> {
  ...
  // 3. Raw screenshots at the four required viewports (reusing the
  // already-loaded desktop context/page).
  ...
  // 4. Overlay screenshots at ~50% opacity -- desktop (concept-01) and
  // phone (concept-04). Uses the real dev-only overlay controls (opacity
  // slider, concept switch) rather than faking the state, then hides the
  // control panel itself just before the shot (the panel is a dev tool,
  // not part of the reviewed composition).
